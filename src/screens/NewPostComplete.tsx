  ...
  const publishPost = () => {
    if (postType != "") {
      if (postTitle != "") {
        ...
        Alert.alert(
          "Titre manquant 🤖",
          "Veuillez rédiger un titre afin de pouvoir publier votre post !",
          [{ text: "Je rédige", style: "default" }]
        );
      }
    } else {
      Alert.alert(
        "Catégorie manquante 🤖",
        "Veuillez sélectionner une catégorie afin de pouvoir publier votre post !",
        [{ text: "Je sélectionne", style: "default" }]
      );
    }
  };
    ...
    <KeyboardAvoidingView style={styles.fullScreen} behavior="padding">
      ...
      <ScrollView style={{ width: "85%", flex: 1 }}>
        ...
        <Text style={styles.photoLocation}>
          Photo prise à : {route.params?.location.latitude},{" "}
          {route.params?.location.longitude} (GPS)
        </Text>
        <Text style={styles.innerTitle}>Choisissez une catégorie</Text>
        <View style={styles.separator}></View>
        <View style={styles.categories}>
          <TouchableOpacity
            style={[styles.category, { backgroundColor: categoryButtonColor1 }]}
            onPress={handleCat1}
          >
            <Text style={styles.categoryText}>Problème</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.category, { backgroundColor: categoryButtonColor2 }]}
            onPress={handleCat2}
          >
            <Text style={styles.categoryText}>Événement</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.category, { backgroundColor: categoryButtonColor3 }]}
            onPress={handleCat3}
          >
            <Text style={styles.categoryText}>Point d'intérêt</Text>
          </TouchableOpacity>
        </View>
        {/* <View style={styles.separator}></View> */}
